...
def find_max_value(diccionario):
    ...
    for nombre, puntaje in diccionario.items():
        if puntaje_maximo is None or puntaje > puntaje_maximo:
...
def reverse_dict(diccionario):
    ...
    """
    invertido = {}
    for clave_vieja, valor_viejo in diccionario.items():
        if valor_viejo in invertido:
            invertido[valor_viejo] += clave_vieja
        else:
            invertido[valor_viejo] = clave_vieja
    return invertido
...
def find_biggest_expense(gastos):
    ...
    for categoria, lista_gastos in gastos.items():
        ...
        cantidad_elementos = len(lista_gastos)
        promedio_actual = suma_total / cantidad_elementos
...
def sum_expenses(gastos):
    ...
    """
    diccionario = {}
    for categoria, listas_gastos in gastos.items():
        suma_total = sum(listas_gastos)
        diccionario[categoria] = suma_total
    return diccionario
